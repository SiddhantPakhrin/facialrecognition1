...
parser.add_argument("-e", "--expressions", default="0,1,2,3,4,5,6",
                    help="choose the facial expression you want to use: 0=Angry, 1=Disgust, 2=Fear, 3=Happy, 4=Sad, "
...
for category in data['Usage'].unique():
        ...
        try:
            if labels[i] in SELECTED_LABELS and nb_images_per_label[get_new_label(labels[i])] < IMAGES_PER_LABEL:
                image = np.fromstring(samples[i], dtype=int, sep=" ").reshape(
                    (image_height, image_width)).astype(np.uint8)
                images.append(image)
                if SAVE_IMAGES:
                    imageio.imwrite(category + '/' + str(i) +
                                    '.jpg', image)
                ...
                labels_list.append(get_new_label(
                    labels[i], one_hot_encoding=ONE_HOT_ENCODING))
    ...
    if ONE_HOT_ENCODING:
        np.save(OUTPUT_FOLDER_NAME + '/' +
                category + '/labels.npy', labels_list)
    else:
        np.save(OUTPUT_FOLDER_NAME + '/' +
                category + '/labels.npy', labels_list)
    if GET_LANDMARKS:
        np.save(OUTPUT_FOLDER_NAME + '/' +
                category + '/landmarks.npy', landmarks)
    # if GET_HOG_FEATURES or GET_HOG_WINDOWS_FEATURES:
    #     np.save(OUTPUT_FOLDER_NAME + '/' + category +
    #             '/hog_features.npy', hog_features)
    #     if GET_HOG_IMAGES:
    #         np.save(OUTPUT_FOLDER_NAME + '/' + category +
    #                 '/hog_images.npy', hog_images)
