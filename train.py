...
def train(optimizer=HYPERPARAMS.optimizer, optimizer_param=HYPERPARAMS.optimizer_param,
    ...
    with tf.Graph().as_default():
        ...
        if train_model:
            ...
            print("  - otimizer_param ({}) = {}".format('beta1' if optimizer ==
                                                        'adam' else 'momentum', optimizer_param))
            ...
            print(
                "  - use batchnorm after conv = {}".format(NETWORK.use_batchnorm_after_conv_layers))
            ...
            if NETWORK.use_landmarks:
                model.fit([data['X'], data['X2']], data['Y'],
                          validation_set=(
                              [validation['X'], validation['X2']], validation['Y']),
            ...
            if TRAINING.save_model:
                ...
                if not(os.path.isfile(TRAINING.save_model_path)) and \
                    ...
                    os.rename(TRAINING.save_model_path +
                              ".meta", TRAINING.save_model_path)

            print("evaluating...")
            validation_accuracy = evaluate(
                model, validation['X'], validation['X2'], validation['Y'])
            print(
            ...
            print(
                "  - evaluation time = {0:.1f} sec".format(time.time() - start_time))
